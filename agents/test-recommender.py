...
# 2. Initialize Graph
if not os.path.exists(TTL_FILENAME):
    print(f"❌ Error: File not found at {TTL_FILENAME}")
    exit()

print(f"Loading Knowledge Graph from: {TTL_FILENAME}...")
initialize_knowledge_graph(TTL_FILENAME)

# 3. Create Agent
recommender = RecommenderAgent()
agent_instance = recommender.create_agent()

# 4. Define Task Logic
def create_recommendation_task(student_interest, agent):
    return Task(
        description=f"""
        1. Search the knowledge graph for courses related to the topic(s): '{student_interest}'.
        2. Identify the top relevant courses.
        3. Return a structured list of these courses including their Course Code and Name.
        """,
        expected_output="A list of JSON objects containing course_code, course_name, and the matching_topic.",
...
if __name__ == "__main__":
    user_interest = input("Enter topic(s) to search for (e.g., AI, Design): ") or "AI"
    print(f"\n🚀 Starting Recommendation Agent for: {user_interest}...\n")

    task = create_recommendation_task(user_interest, agent_instance)
